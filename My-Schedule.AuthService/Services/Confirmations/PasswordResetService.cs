...
namespace My_Schedule.AuthService.Services.Confirmations
{
    public class PasswordResetService
    {
        ...
        private readonly IUserSecurityUpdateService _userAuthDetailUpdateService;
        ...
        public PasswordResetService(ConfirmationService confirmationService, IUserAuthSettings appSettings, IUserSecurityUpdateService userAuthDetailUpdateService, AuthServiceContext dbContext, HashService hashService, NotificationTriggerService notificationTriggerService)
        {
        ...
        public async Task CreatePasswordReset(CredentialsDTO credentialsDTO)
        {
            ...
            {
                var userAuth = await UserSecurityFetcherService.GetUserByEmail(credentialsDTO.Email, _dbContext);

                if (userAuth != null && userAuth.User != null && UserValidator.IsValidUser(userAuth.User, _appSettings.MaxLoginAttempts))
                {
        ...
        public async Task ConfirmPasswordReset(ConfirmDTO confirmDTO)
        {
            ...
            var userAuth = await UserSecurityFetcherService.GetUserById(confirmation.UserId, _dbContext);
            var maxAttempts = _appSettings.MaxLoginAttempts;

            if (userAuth != null && userAuth.User != null && confirmation != null)
            {
                if (UserValidator.IsValidUser(userAuth.User, maxAttempts))
                {
        ...
        private async Task<PasswordReset> GetPasswordReset(Guid userId, Guid confirmationId)
        {
            return await _dbContext.PasswordResets
                ...
                .FirstOrDefaultAsync() ?? throw new ArgumentNullException("password reset");
        }
    }
}